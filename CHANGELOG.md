...
- **Lab feature registry with maturity tiers**: lab-gated features now flow
  through a central registry (`GET /lab_features`) that assigns each feature an
  alpha or beta tier and computes availability server-side. Beta features are
  ...
  frontend environment logic is needed for correctness. The frontend gates all
  lab surfaces (NavBar smb.conf tab, HDIdle badge and disk metrics, HA
  custom-component panel) through a single `useLabFeatures()` hook, and
