...
Use the built-in `new(expr)` for pointers to values. It allocates, assigns the value, and returns a pointer in one expression—replacing helper libraries like `xorcare/pointer` and avoiding `&[]T{v}[0]` hacks.
...
The project targets Go 1.27 (`go.mod`). Apply these idioms; `go fix` can auto-apply most of them (`mise run //backend:format` runs vet/fmt — run `go -C src fix ./...` manually when adopting a new toolchain).

- **UUIDs**: use the top-level standard library `uuid` package (Go 1.27+) instead of `github.com/google/uuid`. `uuid.New()` returns a `[16]byte` value with a `String()` method, so `uuid.New().String()`. Third-party `google/uuid` remains only as an indirect dependency of vendored libraries—do not import it directly.
- **`strings.CutLast(s, sep)`**: prefer over manual `strings.LastIndex` slicing when splitting from the right; it cannot panic on missing separators:
...
- **Modernizer-applied patterns** (safe to write directly): `for i := range n` instead of `for i := 0; i < n; i++`; `strings.SplitSeq` for single-pass iteration over splits; `slices.Backward` for reverse index/value iteration; atomic types (`atomic.Uint64`) instead of raw `atomic.LoadUint64`/`StoreUint64` pairs; embedded struct literal flattening (`events.Event{Type: x}` → `Event{Type: x}` at the outer level).
- **`compress/flate` output is not stable across Go versions** (and its consumers gzip/zip/zlib/png). Never hardcode checksums/digests/sizes of compressed artifacts in tests or code—compute them dynamically from the generated bytes.
- **`go mod tidy` layout**: for modules declaring `go >= 1.27`, tidy merges duplicate require blocks into at most two (direct + indirect). Do not hand-split require blocks back.
- **`go test` runs the stdversion vet check by default**; imports must respect their minimum Go version.
...
- WaitGroup usage by Go version:
  - If `go >= 1.25` in `go.mod` (current: 1.27), use the `WaitGroup.Go` method ([documentation](https://pkg.go.dev/sync#WaitGroup)):
...
- Using pointer helper libraries instead of the built-in `new(expr)` for pointer creation
...
- Importing `github.com/google/uuid` directly instead of the standard library `uuid` package (Go 1.27)
- Slicing with `strings.LastIndex` where `strings.CutLast` expresses intent safely (Go 1.26+)
- Hardcoding digests or sizes of gzip/zip/flate-compressed data in tests—the encoder output changes between Go releases
