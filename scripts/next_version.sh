...
#echo "[next-version] Last tag : ${RAW_TAG}  (major=${MAJOR} minor=${MINOR} patch=${PATCH} pre='${PRE}')" >&2
#echo "[next-version] Today    : year=${CURRENT_YEAR} month=${CURRENT_MONTH}" >&2

# ------------------------------------------------------------------------------
# 4. Year/month correction — takes priority over normal increment rules
# ------------------------------------------------------------------------------
if [[ "$MAJOR" != "$CURRENT_YEAR" || "$MINOR" != "$CURRENT_MONTH" ]]; then
	#echo "[next-version] Year/month mismatch — resetting to ${CURRENT_YEAR}.${CURRENT_MONTH}.0" >&2
...
# ------------------------------------------------------------------------------
echo -n "$VERSION"
#echo "[next-version] VERSION=${VERSION}" >&2
