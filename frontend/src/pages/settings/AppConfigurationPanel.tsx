...
function convertListValues(
  rawValues: unknown[],
  itemConstraint: string,
): unknown[] {
...
function coerceConstraintValue(
  value: unknown,
  ...
  switch (kind) {
    ...
    case "list": {
      ...
      const match = normalizeConstraint(field.constraint).match(
        /^list\((.*)\)$/,
      );
...
function buildRenderableFields(
    ...
    return schemaFields
      .map((field) => {
        ...
        return {
          ...
          optional:
            typeof recordField.optional === "boolean"
              ? recordField.optional
              : isOptionalConstraint(field.constraint),
          options:
            explicitOptions.length > 0 ? explicitOptions : constraintOptions,
        };
      })
  ...
  return Object.keys(options ?? {})
    .map((name) => ({
      name,
      constraint: "str",
      options: optionsFromConstraint("str"),
    }))
    .sort((a, b) => a.name.localeCompare(b.name));
}
...
export function AppConfigurationPanel({
  ...
  const appConfigData =
    appConfig && "options" in appConfig
      ? (appConfig as AppConfigData)
      : undefined;
  const schemaData =
    schema && "fields" in schema ? (schema as AppConfigSchema) : undefined;
  ...
  const showRuntimeConfiguration = useMemo(() => {
    if (
      !appConfigData?.runtime_config ||
      Object.keys(appConfigData.runtime_config).length === 0
    ) {
      return false;
    }

    return !deepEqualValues(
      appConfigData.options,
      appConfigData.runtime_config,
    );
  }, [appConfigData?.options, appConfigData?.runtime_config]);

  const initialValues = useMemo(
    () => normalizeFormValues(appConfigData?.options),
    [appConfigData?.options],
  );
        ...
        <Box id="app-config-form">
          <Stack spacing={3}>
            {fields.map((field) => {
              ...
              const kind = getConstraintKind(field.constraint);
              const required = !(
                field.optional ?? isOptionalConstraint(field.constraint)
              );
            ...
            >
              <Button
                disabled={!formState.isDirty || updateState.isLoading}
              ...
              <Button
                ...
                disabled={
                  !formState.isDirty || updateState.isLoading || readOnly
                }
              >
