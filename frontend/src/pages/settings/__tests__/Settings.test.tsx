...
describe("Settings", () => {
    ...
    it("shows install confirmation dialog when Install button is clicked", async () => {
        ...
        render(
            React.createElement(
                Provider,
                { store, children: React.createElement(ThemeProvider, { theme }, React.createElement(Settings as any)) }
            )
        );

        const user = userEvent.setup();
        const haTreeItem = await screen.findByRole("treeitem", { name: /homeassistant/i });
        await user.click(within(haTreeItem).getByText(/homeassistant/i));

        const installButton = await screen.findByRole("button", { name: /^install$/i });
        await user.click(installButton);

        // Confirmation dialog must appear: verify via action buttons (reliable role query)
    ...
    it("dismisses confirmation dialog when Cancel is clicked", async () => {
        ...
        // Dialog should be dismissed and no action feedback shown
    ...
    it("dismisses restart dialog when Later is clicked", async () => {
        ...
        // Dialog should be dismissed (Restart Now button no longer present)
