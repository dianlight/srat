...
export function Shares() {
  ...
  const [selectedShareKey, setSelectedShareKey] = useState<string | undefined>(
    () => localStorage.getItem("shares.selectedShareKey") || undefined,
  );
  const [selectedShare, setSelectedShare] = useState<SharedResource | null>(
    null,
  );
  ...
  const [disableShare, _updateDisableShareResult] =
    usePutApiShareByShareNameDisableMutation();
  ...
  const handleShareSelect = useCallback(
    (shareKey: string, share: SharedResource) => {
      setSelectedShareKey(shareKey);
  ...
  useEffect(() => {
    ...
    const disposeSharesStep3 = TourEvents.on(
      TourEventTypes.SHARES_STEP_3,
      selectTourShare,
    );
    const disposeSharesStep4 = TourEvents.on(
