...
export function ShareEditForm({
  ...
  const adminUser = useMemo(() => {
    return Array.isArray(users) ? users.find((u) => u.is_admin) : undefined;
  }, [users]);
  ...
  const { control, handleSubmit, watch, setValue, getValues } =
    useForm<ShareEditProps>({
      defaultValues: {
        ...
        users: shareData?.mount_point_data?.is_write_supported
          ? shareData.org_name === undefined &&
            (!shareData?.users || shareData.users.length === 0) &&
            adminUser
            ? [adminUser]
            : shareData.users || []
          : [],
        ro_users: shareData?.mount_point_data?.is_write_supported
          ? shareData?.ro_users || []
          : shareData?.org_name === undefined &&
              (!shareData?.ro_users || shareData.ro_users.length === 0) &&
              adminUser
            ? [adminUser]
            : shareData?.ro_users || [],
        timemachine:
          shareData?.mount_point_data?.time_machine_support ===
          Time_machine_support.Unsupported
            ? false
            : shareData?.timemachine || false,
        recycle_bin_enabled: shareData?.recycle_bin_enabled || false,
        guest_ok: shareData?.guest_ok || false,
        timemachine_max_size:
          shareData?.timemachine_max_size ||
          (shareData?.mount_point_data?.disk_size
            ? filesize(shareData?.mount_point_data?.disk_size, {
                round: 0,
              })
            : "MAX"),
  ...
  const [availablePartitions, setAvailablePartition] = useState<
    MountPointData[]
  >([]);

  useEffect(() => {
    if (volumes) {
      const newAvailablePartitions =
        (volumes
          ...
          .flatMap((partition) =>
            Object.values(partition?.mount_point_data || {}),
          )
          .filter((mp) => mp?.path !== "") as MountPointData[]) || [];
                  ...
                  autocompleteProps={{
                    ...
                    renderValue: (value: MountPointData) => {
                      return (
                        <Typography variant="body2">
                          {value.disk_label || value.device_id}{" "}
                          <sup>
                            {value.is_write_supported ? (
                              ""
                            ) : (
                    ...
                    getOptionDisabled: (option) => {
                      ...
                      for (const existingShare of Object.values(shares)) {
                        if (
                          existingShare.mount_point_data?.path === option.path
                        ) {
                          if (
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              <SelectElement
                ...
                options={Object.values(Usage)
                  .filter((usage) => usage !== Usage.Internal)
                  .map((usage) => {
                    return {
                      id: usage,
                      label: usage.charAt(0).toUpperCase() + usage.slice(1),
                    };
                  })}
                required
                control={control}
              ...
              render={({ field, fieldState: { error } }) => (
                <MuiChipsInput
                  ...
                  renderChip={(Component, key, props) => {
                    const isDefault = default_json.veto_files?.includes(
                                ...
                                onClick={() => {
                                  ...
                                  const newVetoFilesToAdd =
                                    defaultVetoFiles.filter(
                                      (defaultFile) =>
                                        !currentVetoFiles.includes(defaultFile),
                                    );
                                  setValue(
                                    "veto_files",
                                    [...currentVetoFiles, ...newVetoFilesToAdd],
                                    {
                                      shouldDirty: true,
                                      shouldValidate: true,
                                    },
              ...
              <TextFieldElement
                ...
                rules={{
                  pattern: {
                    ...
                    message:
                      "Invalid format. Use MAX or a number followed by K, M, G, T, P (e.g., 100G, 5T).",
                  },
                }}
              />
            </Grid>
              ...
              {!usLoading && ((users as User[]) || []).length > 0 && (
                <AutocompleteElement
                  ...
                  autocompleteProps={{
                    disabled:
                      isDisabled ||
                      watch("mount_point_data")?.is_write_supported === false,
                    size: "small",
                    limitTags: 3,
                    ...
                    renderValue: (values, getItemProps) =>
                      values.map((option, index) => {
                        ...
                        return (
                          <Chip
                            color={
                              (option as User).is_admin ? "warning" : "default"
                            }
                            key={key}
                            variant="outlined"
                            label={(option as User)?.username || "unknown"}
                            size="small"
                            {...itemProps}
                          />
                        );
  ...
  const renderNameHeader = () => (
    ...
    >
      {!(editName || shareData?.org_name === undefined) && (
        <Box
          sx={{ display: "flex", alignItems: "center", flexGrow: "inherit" }}
        >
          <Tooltip
            title={
              shareData?.usage === Usage.Internal
                ? "Cannot edit name of internal shares"
                : "Edit share name"
            }
          >
            <span>
