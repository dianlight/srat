...
function SmartStatusPanelInner({
  ...
  const {
    ...
    isSuccess: smartOperationSuccess,
  } = useSmartOperations(diskId);
  const {
    data: smartStatus,
    isLoading: smartStatusIsLoading,
    refetch: refetchSmartStatus,
  } = useGetApiDiskByDiskIdSmartStatusQuery(
    {
      diskId: diskId || "",
    },
              ...
              <Box>
                ...
                <Stack spacing={1}>
                  ...
                  >
                    ...
                    {smartTestStatus?.running &&
                      smartTestStatus.percent_complete !== undefined && (
                        <Box
        ...
        <DialogContent sx={{ pt: 2 }}>
          <FormControl fullWidth>
            ...
            <Select
              ...
              label="Test Type"
              onChange={(e) =>
                setSelectedTestType(e.target.value as SmartTestType)
              }
            >
              <MenuItem value="short">Short (2-5 minutes)</MenuItem>
              <MenuItem value="long">Long (hours - full disk scan)</MenuItem>
              <MenuItem value="conveyance">
                Conveyance (test for transport damage)
              </MenuItem>
            </Select>
          </FormControl>
          <Typography
          ...
          {bus?.toLowerCase() === "usb" && (
            ...
            >
              Warning: Running SMART self-tests on USB-connected drives may
              cause them to disconnect temporarily. Ensure you have backups of
              any important data before proceeding. Also note that SMART test
              can't be performed or results may be unreliable for USB drives due
              to limitations in how SMART data is reported over USB interfaces.
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
