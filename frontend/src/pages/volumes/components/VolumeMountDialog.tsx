...
export function VolumeMountDialog(props: VolumeMountDialogProps) {
  ...
  const [unsupported_custom_flags, setUnsupportedCustomFlags] = useState<
    MountFlag[]
  >([]); // Array of unsupported custom flags (string) for display only

  const [customMountFlagsOptions, setCustomMountFlagsOptions] = useState<
    MountFlag[]
  >([]);
  ...
  useEffect(() => {
    if (props.open && !fsLoading) {
      const selectedFilesystem = (
        filesystems as FilesystemsInfo
      )?.filesystems?.find((fs) => fs.type === fstype);
    ...
    if (props.open && props.objectToEdit) {
      ...
      if (existingMountData?.fstype) {
        ...
        const fsCurrent = fsLoading
          ? undefined
          : (filesystems as FilesystemsInfo)?.filesystems?.find(
              (fs) => fs.type === existingMountData.fstype,
            );
        if (fsCurrent) {
          ...
          existingMountData?.flags?.forEach((flag) => {
            ...
            if (
              !(filesystems as FilesystemsInfo)?.mount_flags?.find(
                (flagItem) => flagItem.name === flag.name,
              )
            ) {
          ...
          existingMountData?.custom_flags?.forEach((flag) => {
            ...
            if (
              !fsCurrent.custom_mount_flags?.find(
                (flagItem) => flagItem.name === flag.name,
              )
            ) {
      ...
      reset({
        ...
        fstype:
          existingMountData?.fstype || props.objectToEdit?.fs_type || undefined, // Use existing or let backend detect
  ...
  const partitionNameDecoded = decodeEscapeSequence(
    props.objectToEdit?.name ||
      props.objectToEdit?.legacy_device_name ||
      "Unnamed Partition",
  );
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ pt: 1 }}>
            ...
            <Grid container spacing={2}>
              <Grid size={{ xs: 12, sm: 12 }}>
                <AutocompleteElement
                  ...
                  options={
                    fsLoading
                      ? []
                      : (filesystems as FilesystemsInfo)?.filesystems?.map(
                          (fs) => ({
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                {!fsLoading && customMountFlagsOptions.length > 0 && (
                  <AutocompleteElement<
                    MountFlag,
                    true,
                    false,
                  ...
                  >
                    ...
                    options={fsLoading ? [] : customMountFlagsOptions}
                    control={control}
                    autocompleteProps={{
                      disabled: props.readOnlyView,
                      size: "small",
                      ...
                      onChange: props.readOnlyView
                        ...
                        : (_event, value) => {
                            ...
                            const newFieldValues = currentFieldValues.filter(
                              (fv) =>
                                flagsWithValue.some(
                                  (selectedFlag) =>
                ...
                <Grid size={{ xs: 12, sm: 6 }} key={`${field.id}_edit_`}>
                  <TextFieldElement
                    ...
                    rules={{
                      ...
                      pattern: {
                        value: new RegExp(field.value_validation_regex || ".*"),
