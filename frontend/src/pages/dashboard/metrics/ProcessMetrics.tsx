...
export function ProcessMetrics({
  ...
  for (const subprocess of subprocesses) {
    const parentPid = Math.abs(subprocess.pid ?? 0);
  ...
  const renderProcess = (
    ...
    const pidDisplay =
      isSubprocess && process.pid !== null && process.pid <= 0
        ? "sub"
    ...
    return (
      <TableRow key={tableRowKey}>
        ...
        <TableCell
          align="right"
          sx={{
            color: process.status === "Running" ? "success.main" : "error.main",
          }}
        >
          {process.status}
        </TableCell>
          ...
          <TableBody>
            {mainProcesses.flatMap((process, processIndex) => {
              ...
              if (
                ...
                subprocessMap
                  .get(process.pid)
                  ?.forEach((subprocess, subIndex) => {
                    rows.push(
                      renderProcess(
                        subprocess,
                        true,
                        `process-${processIndex}-sub-${subIndex}`,
                      ),
                    );
                  });
              }
                ...
                process.child_processes.forEach((child, childIndex) => {
                  rows.push(
                    renderProcess(
                      child,
                      true,
                      `process-${processIndex}-child-${childIndex}`,
                    ),
                  );
                });
              }
