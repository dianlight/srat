...
export function DiskHealthMetrics({
  ...
  const isDiskIoStats = (obj: unknown): obj is DiskIoStats =>
    !!obj &&
    typeof obj === "object" &&
    "device_name" in obj &&
    "device_description" in obj;

  const sortedDiskIo = [...(diskHealth?.per_disk_io ?? [])].sort((a, b) =>
    (a.device_name || "").localeCompare(b.device_name || ""),
  );

  const sortedPartitionEntries = Object.entries(
    diskHealth?.per_partition_info || {},
  ...
  return (
    <>
        ...
        <Table aria-label="disk health table" size="small">
          ...
          <TableBody>
            {sortedDiskIo.map((io) => {
              ...
              const diskInfo =
                diskHealth?.per_disk_info?.[io.device_description];
              const smartHealth = diskInfo?.smart_health;
              const isSmartHealthOk = !smartHealth || smartHealth.passed;
              const smartHealthTooltip =
                smartHealth && !smartHealth.passed
                ...
                <TableRow key={io.device_name}>
                  ...
                  <TableCell align="right">
                    <Typography variant="body2">
                      {io.smart_data?.power_on_hours?.value?.toLocaleString() ??
                        "N/A"}
                    </Typography>
                  </TableCell>
                  <TableCell align="right">
                    <Typography variant="body2">
                      {io.smart_data?.power_cycle_count?.value?.toLocaleString() ??
                        "N/A"}
                    </Typography>
                  </TableCell>
                </TableRow>
      ...
      <Grid container spacing={2}>
        {sortedPartitionEntries.map(([diskName, partitions]) => (
                    ...
                    return (
                      <div key={partition.device} style={{ marginTop: "16px" }}>
                        <Typography
                          variant="subtitle2"
                          sx={{ cursor: "pointer" }}
                          onClick={() => setSelectedIoStats(partition)}
                        >
