...
export function ProcessMetricsAccordion({
  ...
  const aggregate = (() => {
    ...
    for (const p of processData) {
      ...
      if (typeof p.cpu === "number" && !Number.isNaN(p.cpu)) cpu += p.cpu;
      if (typeof p.memory === "number" && !Number.isNaN(p.memory))
        memory += p.memory;
      if (typeof p.connections === "number" && !Number.isNaN(p.connections))
