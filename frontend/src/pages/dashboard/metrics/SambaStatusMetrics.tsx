                  ...
                  <Fragment key={key}>
                    <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
                      ...
                      <TableCell align="right">
                        {Object.entries(session.channels || {}).map(
                          ([channelKey, channel]) => (
                            <span key={channelKey}>
                              {channel.channel_id}({channel.transport}){" "}
                            </span>
                          ),
                        )}
                      </TableCell>
                      ...
                      <TableCell>
                        {session.encryption?.cipher}{" "}
                        {session.encryption?.degree}
                      </TableCell>
                      <TableCell>
                        {session.signing?.cipher} {session.signing?.degree}
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell
                        ...
                        <Collapse in={isOpen} timeout="auto" unmountOnExit>
                          <Box sx={{ margin: 1 }}>
                            ...
                            {relatedTcons.length > 0 ? (
                              <Table
                                size="small"
                                aria-label={`samba tcons subtable ${session.session_id}`}
                              >
                                <TableHead>
                                  <TableRow>
                                ...
                                <TableBody>
                                  {relatedTcons.map(([tconKey, tcon]) => (
                                    <TableRow key={tconKey}>
                                      ...
                                      <TableCell>
                                        {tcon.encryption?.cipher}{" "}
                                        {tcon.encryption?.degree}
                                      </TableCell>
                                      <TableCell>
                                        {tcon.signing?.cipher}{" "}
                                        {tcon.signing?.degree}
