...
export function UserEditForm({
  ...
  const handleFormSubmit = (data: UsersProps) => {
    const uniqueRwShares = Array.from(
      new Set((data.rw_shares || []).filter(Boolean)),
    );
    const uniqueRoShares = Array.from(
      new Set((data.ro_shares || []).filter(Boolean)),
              ...
              <Typography variant="body2" color="text.secondary">
                {isAdmin ? (
                  <>
                    ...
                    admin username and password. The admin account cannot be
                    deleted, but there can only be one admin user.
                  </>
                ) : isNewUser ? (
                  <>
                    <strong>New User:</strong> Create a new regular user
                    account. Users can be assigned read/write or read-only
                    access to shares.
                  </>
                ) : (
                  <>
                    <strong>Regular User:</strong> You can update the password
                    for this user. The username cannot be changed after
                    creation.
                  </>
                )}
              </Typography>
