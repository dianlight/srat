...
const ALL_TAB_CONFIGS: TabConfig[] = [
  {
    ...
    label: "Dashboard",
    component: <Dashboard />,
    tutorialSteps: DashboardSteps,
  },
  {
    id: TabIDs.VOLUMES,
    label: "Volumes",
    component: <Volumes />,
    tutorialSteps: VolumesSteps,
  },
  {
    id: TabIDs.SHARES,
    label: "Shares",
    component: <Shares />,
    tutorialSteps: SharesSteps,
  },
  {
    id: TabIDs.USERS,
    label: "Users",
    component: <Users />,
    tutorialSteps: UsersSteps,
  },
  {
    id: TabIDs.SETTINGS,
    label: "Settings",
    component: <Settings />,
    tutorialSteps: SettingsSteps,
  },
  {
    id: TabIDs.SMB_FILE_CONFIG,
    label: "smb.conf",
    component: <SmbConfPage />,
    labFeatureKey: "smb_conf",
...
const getTabIcon = (tab: TabConfig, healthData: HealthPing | undefined) => {
  ...
  if (healthData?.dirty_tracking) {
    const dirtyMap: Partial<Record<TabIDs, keyof DataDirtyTracker>> = {
...
function TabPanel(props: TabPanelProps) {
  ...
  }, [isTourOpen, tutorialSteps, value, setSteps, index]);
  ...
  const visibleTabs = useMemo(() => {
    return ALL_TAB_CONFIGS.filter(
      (tab) =>
        ...
        !(tab.labFeatureKey && !labFeatureAvailable(tab.labFeatureKey)),
    ).map((tab, index) => ({
      ...tab,
      actualIndex: index,
    }));
  ...
  useEffect(() => {
    if (evdata && evdata !== prevEvdataRef.current) {
      ...
      for (const key of Object.keys(evdata) as Array<keyof typeof evdata>) {
        if (
          !prevData ||
          JSON.stringify(evdata[key]) !== JSON.stringify(prevData[key])
        ) {
          ...
          >
            ...
            <Box sx={{ flexGrow: 0, display: "flex", alignItems: "center" }}>
                      ...
                      >
                        <ListItem>
                          <ListItemText
                            ...
                            secondary={
                              evdata?.hello?.protected_mode === true
                                ? "Enabled"
                                : "Disabled"
                            }
                          />
                        ...
                        >
                          <span>
                            SSE Messages {ignoreHeartbeat ? "(!heartbeat)" : ""}
                          </span>
                          <Tooltip
                            title="Heartbeat messages are sent frequently (~1/sec). Enable to filter them out and see other event types."
                            arrow
                          >
                            <Switch
                              size="small"
                        ...
                        {filteredMessages.length === 0 ? (
                          <ListItem>
                            <ListItemText
                              ...
                              secondary="Waiting for SSE events..."
                            />
                          </ListItem>
                        ) : (
                          filteredMessages.map((msg) => (
                            <ListItem key={msg.timestamp} sx={{ pr: 8 }}>
                              <ListItemText
              ...
              >
                <Tooltip
                  title={
                    issueTemplateAvailable ? "GitHub Menu" : "View on GitHub"
                  }
                  arrow
                >
