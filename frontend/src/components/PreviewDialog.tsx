      ...
      <TreeItem
        ...
        label={
          <Box component="span">
            ...
            <Box
              component="span"
              sx={{ color: "text.secondary", fontSize: "0.875em" }}
            >
              {" "}
              ({typeof value}
              {isSensitive ? ", censored" : ""})
            </Box>
          </Box>
        }
      />
    ...
    return (
      ...
      >
        {value.map((item, index) => (
          <ObjectTreeNode
            // biome-ignore lint/suspicious/noArrayIndexKey: array items have no unique id
            key={`${nodeId}.${index}`}
            value={item}
            nodeId={`${nodeId}.${index}`}
            label={`[${index}]`}
...
export function ObjectTree(props: { object: unknown }) {
  ...
  return (
    ...
    >
      <ObjectTreeNode value={props.object} nodeId="root" label="Root" />
    </SimpleTreeView>
  );
}
