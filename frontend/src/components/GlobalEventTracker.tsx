...
export class LogEntry {
  ...
  constructor(source: string, level: string, args: unknown[]) {
    this.source = source;
    this.level = level;
...
class GlobalEventTracker {
  ...
  history: LogEntry[] = [];
  maxSize: number = 200;
  listeners: Set<(history: LogEntry[]) => void> = new Set();
  ...
  initConsole() {
    ["log", "info", "warn", "error"].forEach((level) => {
      const method = console[level as keyof Console] as (
        ...args: unknown[]
      ) => void;
  ...
  initGlobalListeners() {
    window.addEventListener(
      ...
      (event) => {
        ...
        this.addEntry("BROWSER_RUNTIME", "error", [
          message,
          `Origine: ${source}`,
        ]);
      },
    ...
    window.addEventListener("unhandledrejection", (event) => {
      this.addEntry("UNHANDLED_PROMISE", "error", [
        event.reason || "Nessuna ragione fornita",
      ]);
    });
  }
  ...
  initFetchInterceptor() {
    ...
    const wrappedFetch = async (...args: Parameters<typeof fetch>) => {
      try {
        ...
        if (!response.ok) {
          this.addEntry("NETWORK_FETCH", "error", [
            `HTTP ${response.status} su ${args[0]}`,
          ]);
        }
        return response;
      } catch (err) {
        ...
        this.addEntry("NETWORK_FETCH", "error", [
          `Fallimento connessione: ${errorMessage}`,
        ]);
        throw err;
      }
    };
  ...
  addEntry(source: string, level: string, args: unknown[]) {
