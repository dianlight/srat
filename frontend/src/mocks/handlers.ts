...
export const handlers = [
	...
	http.get(`${baseURL}/api/problems`, async () => {
		...
		const resultArray = [
			[
				[
					{
						id: 1,
						problem_key: "mock_problem_disk_warning",
						title: "Disk requires attention",
						description: "Mock problem generated by MSW",
						severity: "warning",
						status: "created",
						ignored: false,
						repeating: 0,
			...
			[{ detail: "mock problem error" }, { status: 500 }],
		] as [any, { status: number }][];

		return HttpResponse.json(
			...resultArray[next(`get /api/problems`) % resultArray.length],
		);
	}),
	http.delete(`${baseURL}/api/problems/:problemKey`, async () => {
		const resultArray = [
			[undefined, { status: 204 }],
		...
		return HttpResponse.json(
			...resultArray[
			next(`delete /api/problems/:problemKey`) % resultArray.length
			],
		);
	}),
