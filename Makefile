...
ALL:
	$(MAKE) -C $(BACKEND_DIRS) build AARGS="GOARCH=amd64" VERSION=$(VERSION) ARCH="x86_64" ROLLBAR_CLIENT_ACCESS_TOKEN="$(ROLLBAR_CLIENT_ACCESS_TOKEN)" ROLLBAR_ENVIRONMENT="$(ROLLBAR_ENVIRONMENT)" GIST_TOKEN="$(GIST_TOKEN)"
	$(MAKE) -C $(BACKEND_DIRS) build AARGS="GOARCH=arm64" VERSION=$(VERSION) ARCH="aarch64" ROLLBAR_CLIENT_ACCESS_TOKEN="$(ROLLBAR_CLIENT_ACCESS_TOKEN)" ROLLBAR_ENVIRONMENT="$(ROLLBAR_ENVIRONMENT)" GIST_TOKEN="$(GIST_TOKEN)"
...
docs-toc:
	...
	@find . -name "*.md" -not -path "./node_modules/*" -not -path "./frontend/node_modules/*" -not -path "./backend/src/vendor/*" -not -path "./.vale/*" -not -path "./.github/*" -not -path "./docs/tasks/*" | xargs bunx doctoc --github
...
docs-fix:
	...
	bunx markdownlint-cli2 "**/*.md" "#frontend/node_modules" "#backend/src/vendor" "#.vale" "#.github" "#docs/tasks" --fix
