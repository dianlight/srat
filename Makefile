...
ALL:
	$(MAKE) -C $(BACKEND_DIRS) build AARGS="GOARCH=amd64 CC='zig cc -target x86_64-linux-musl'" VERSION=$(VERSION) ARCH="x86_64" ROLLBAR_CLIENT_ACCESS_TOKEN="$(ROLLBAR_CLIENT_ACCESS_TOKEN)" ROLLBAR_ENVIRONMENT="$(ROLLBAR_ENVIRONMENT)"
	$(MAKE) -C $(BACKEND_DIRS) build AARGS="GOARCH=arm64 CC='zig cc -target aarch64-linux-musl'" VERSION=$(VERSION) ARCH="aarch64" ROLLBAR_CLIENT_ACCESS_TOKEN="$(ROLLBAR_CLIENT_ACCESS_TOKEN)" ROLLBAR_ENVIRONMENT="$(ROLLBAR_ENVIRONMENT)"
