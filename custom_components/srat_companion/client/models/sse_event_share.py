...
T = TypeVar("T", bound="SseEventShare")
...
class SseEventShare:
    ...
    Attributes:
        data (Union[None, list['SharedResource']]):
        event (Literal['share']): The event name.
        id (Union[Unset, int]): The event ID.
        retry (Union[Unset, int]): The retry time in milliseconds.

    """

    data: None | list["SharedResource"]
    event: Literal["share"]
    id: Unset | int = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        def _parse_data(data: object) -> None | list["SharedResource"]:
            if data is None:
                return data
            try:
                ...
                for data_type_0_item_data in _data_type_0:
                    data_type_0_item = SharedResource.from_dict(data_type_0_item_data)
        ...
        event = cast("Literal['share']", d.pop("event"))
        if event != "share":
            raise ValueError(f"event must match const 'share', got '{event}'")
        ...
        sse_event_share.additional_properties = d
        return sse_event_share
