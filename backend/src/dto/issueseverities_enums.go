...
// It is private; use the exported IssueSeverity values and methods instead.
...
// Otherwise, it returns nil
func numberToIssueSeverity[T interface {
	...
	switch f {
	case 0:
		result := IssueSeverities.ISSUESEVERITYERROR
		if !result.IsValid() {
			return nil
		}
		return &result
	case 1:
		result := IssueSeverities.ISSUESEVERITYWARNING
		if !result.IsValid() {
			return nil
		}
		return &result
	case 2:
		result := IssueSeverities.ISSUESEVERITYINFO
		if !result.IsValid() {
			return nil
		}
		return &result
	case 3:
		result := IssueSeverities.ISSUESEVERITYSUCCESS
		if !result.IsValid() {
			return nil
		}
		return &result
...
type IssueSeverityMatcher interface {
	IssueSeverityError()
...
// MatchIssueSeverity dispatches to the matcher method for the given enum value.
// The IssueSeverityMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchIssueSeverity(en IssueSeverity, matcher IssueSeverityMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil IssueSeverityMatcher")
	}
	switch en {
	case IssueSeverities.ISSUESEVERITYERROR:
		matcher.IssueSeverityError()
	case IssueSeverities.ISSUESEVERITYWARNING:
		matcher.IssueSeverityWarning()
	case IssueSeverities.ISSUESEVERITYINFO:
		matcher.IssueSeverityInfo()
	case IssueSeverities.ISSUESEVERITYSUCCESS:
		matcher.IssueSeveritySuccess()
	default:
		return fmt.Errorf("unhandled IssueSeverity: %v", en)
	}
	return nil
}

// MustMatchIssueSeverity dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchIssueSeverity(en IssueSeverity, matcher IssueSeverityMatcher) {
	if err := MatchIssueSeverity(en, matcher); err != nil {
		panic(err)
	}
}
...
// issueseverityNames is a constant string containing the canonical names for all enum values.
