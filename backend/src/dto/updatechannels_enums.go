...
// It is private; use the exported UpdateChannel values and methods instead.
...
// Otherwise, it returns nil
func numberToUpdateChannel[T interface {
	...
	switch f {
	case 0:
		result := UpdateChannels.NONE
		if !result.IsValid() {
			return nil
		}
		return &result
	case 1:
		result := UpdateChannels.DEVELOP
		if !result.IsValid() {
			return nil
		}
		return &result
	case 2:
		result := UpdateChannels.RELEASE
		if !result.IsValid() {
			return nil
		}
		return &result
	case 3:
		result := UpdateChannels.PRERELEASE
		if !result.IsValid() {
			return nil
		}
		return &result
...
func (u *UpdateChannel) UnmarshalJSON(by []byte) error {
	var value string
	...
	newu, err := ParseUpdateChannel(by)
	if err != nil {
		return err
	}
	*u = newu
...
func (u *UpdateChannel) UnmarshalText(by []byte) error {
	newu, err := ParseUpdateChannel(by)
	if err != nil {
		return err
	}
...
// It returns an error if the byte slice does not contain a valid enum value.
func (u *UpdateChannel) UnmarshalBinary(by []byte) error {
	newu, err := ParseUpdateChannel(by)
	if err != nil {
		return err
	}
...
func (u *UpdateChannel) UnmarshalYAML(by []byte) error {
	newu, err := ParseUpdateChannel(by)
	if err != nil {
		return err
	}
...
// updatechannelNames is a constant string containing the canonical names for all enum values.
