...
// It is private; use the exported TimeMachineSupport values and methods instead.
...
// Otherwise, it returns nil
func numberToTimeMachineSupport[T interface {
	int | int8 | int16 | int32 | int64 | uint | uint8 | uint16 | uint32 | uint64 | uintptr | float32 | float64
}](num T) *TimeMachineSupport {
	f := float64(num)
	if math.Floor(f) != f {
		return nil
	}
	switch f {
	case 0:
		result := TimeMachineSupports.UNSUPPORTED
		if !result.IsValid() {
			return nil
		}
		return &result
	case 1:
		result := TimeMachineSupports.SUPPORTED
		if !result.IsValid() {
			return nil
		}
		return &result
	case 2:
		result := TimeMachineSupports.EXPERIMENTAL
		if !result.IsValid() {
			return nil
		}
		return &result
	case 3:
		result := TimeMachineSupports.UNKNOWN
		if !result.IsValid() {
			return nil
		}
		return &result
...
type TimeMachineSupportMatcher interface {
	Unsupported()
	Supported()
	Experimental()
	Unknown()
...
// MatchTimeMachineSupport dispatches to the matcher method for the given enum value.
// The TimeMachineSupportMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchTimeMachineSupport(en TimeMachineSupport, matcher TimeMachineSupportMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil TimeMachineSupportMatcher")
	}
	switch en {
	case TimeMachineSupports.UNSUPPORTED:
		matcher.Unsupported()
	case TimeMachineSupports.SUPPORTED:
		matcher.Supported()
	case TimeMachineSupports.EXPERIMENTAL:
		matcher.Experimental()
	case TimeMachineSupports.UNKNOWN:
		matcher.Unknown()
	default:
		return fmt.Errorf("unhandled TimeMachineSupport: %v", en)
	}
	return nil
}

// MustMatchTimeMachineSupport dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchTimeMachineSupport(en TimeMachineSupport, matcher TimeMachineSupportMatcher) {
	if err := MatchTimeMachineSupport(en, matcher); err != nil {
		panic(err)
	}
}
...
func (t *TimeMachineSupport) UnmarshalJSON(by []byte) error {
	var value string
	...
	newt, err := ParseTimeMachineSupport(by)
	if err != nil {
		return err
	}
	*t = newt
...
func (t *TimeMachineSupport) UnmarshalText(by []byte) error {
	newt, err := ParseTimeMachineSupport(by)
	if err != nil {
		return err
	}
...
// It returns an error if the byte slice does not contain a valid enum value.
func (t *TimeMachineSupport) UnmarshalBinary(by []byte) error {
	newt, err := ParseTimeMachineSupport(by)
	if err != nil {
		return err
	}
...
func (t *TimeMachineSupport) UnmarshalYAML(by []byte) error {
	newt, err := ParseTimeMachineSupport(by)
	if err != nil {
		return err
	}
...
// timemachinesupportNames is a constant string containing the canonical names for all enum values.
