...
// It is private; use the exported TelemetryMode values and methods instead.
...
// Otherwise, it returns nil
func numberToTelemetryMode[T interface {
	...
	switch f {
	case 0:
		result := TelemetryModes.TELEMETRYMODEASK
		if !result.IsValid() {
			return nil
		}
		return &result
	case 1:
		result := TelemetryModes.TELEMETRYMODEALL
		if !result.IsValid() {
			return nil
		}
		return &result
	case 2:
		result := TelemetryModes.TELEMETRYMODEERRORS
		if !result.IsValid() {
			return nil
		}
		return &result
	case 3:
		result := TelemetryModes.TELEMETRYMODEDISABLED
		if !result.IsValid() {
			return nil
		}
		return &result
...
type TelemetryModeMatcher interface {
	TelemetryModeAsk()
	TelemetryModeAll()
	TelemetryModeErrors()
	TelemetryModeDisabled()
}

// MatchTelemetryMode dispatches to the matcher method for the given enum value.
// The TelemetryModeMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchTelemetryMode(en TelemetryMode, matcher TelemetryModeMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil TelemetryModeMatcher")
	}
	switch en {
	case TelemetryModes.TELEMETRYMODEASK:
		matcher.TelemetryModeAsk()
	case TelemetryModes.TELEMETRYMODEALL:
		matcher.TelemetryModeAll()
	case TelemetryModes.TELEMETRYMODEERRORS:
		matcher.TelemetryModeErrors()
	case TelemetryModes.TELEMETRYMODEDISABLED:
		matcher.TelemetryModeDisabled()
	default:
		return fmt.Errorf("unhandled TelemetryMode: %v", en)
	}
	return nil
}

// MustMatchTelemetryMode dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchTelemetryMode(en TelemetryMode, matcher TelemetryModeMatcher) {
	if err := MatchTelemetryMode(en, matcher); err != nil {
		panic(err)
	}
}
...
// telemetrymodeNames is a constant string containing the canonical names for all enum values.
