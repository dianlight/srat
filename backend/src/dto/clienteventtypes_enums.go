...
// It is private; use the exported ClientEventType values and methods instead.
...
// Otherwise, it returns nil
func numberToClientEventType[T interface {
	int | int8 | int16 | int32 | int64 | uint | uint8 | uint16 | uint32 | uint64 | uintptr | float32 | float64
}](num T) *ClientEventType {
	f := float64(num)
	if math.Floor(f) != f {
		return nil
	}
	switch f {
	case 0:
		result := ClientEventTypes.CLIENTEVENTTYPEHELO
		if !result.IsValid() {
			return nil
		}
		return &result
	case 1:
		result := ClientEventTypes.CLIENTEVENTTYPEREPAIRLIFECYCLE
		if !result.IsValid() {
			return nil
		}
		return &result
	case 2:
		result := ClientEventTypes.CLIENTEVENTTYPEPROBLEMLIFECYCLE
		if !result.IsValid() {
			return nil
		}
		return &result
