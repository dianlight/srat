...
// It is private; use the exported UpdateProcessState values and methods instead.
...
// Otherwise, it returns nil
func numberToUpdateProcessState[T interface {
	int | int8 | int16 | int32 | int64 | uint | uint8 | uint16 | uint32 | uint64 | uintptr | float32 | float64
}](num T) *UpdateProcessState {
	f := float64(num)
	if math.Floor(f) != f {
		return nil
	}
	switch f {
	case 0:
		result := UpdateProcessStates.UPDATESTATUSIDLE
		if !result.IsValid() {
			return nil
		}
		return &result
	case 1:
		result := UpdateProcessStates.UPDATESTATUSCHECKING
		if !result.IsValid() {
			return nil
		}
		return &result
	case 2:
		result := UpdateProcessStates.UPDATESTATUSNOUPGRADE
		if !result.IsValid() {
			return nil
		}
		return &result
	case 3:
		result := UpdateProcessStates.UPDATESTATUSUPGRADEAVAILABLE
		if !result.IsValid() {
			return nil
		}
		return &result
	case 4:
		result := UpdateProcessStates.UPDATESTATUSDOWNLOADING
		if !result.IsValid() {
			return nil
		}
		return &result
	case 5:
		result := UpdateProcessStates.UPDATESTATUSDOWNLOADCOMPLETE
		if !result.IsValid() {
			return nil
		}
		return &result
	case 6:
		result := UpdateProcessStates.UPDATESTATUSEXTRACTING
		if !result.IsValid() {
			return nil
		}
		return &result
	case 7:
		result := UpdateProcessStates.UPDATESTATUSEXTRACTCOMPLETE
		if !result.IsValid() {
			return nil
		}
		return &result
	case 8:
		result := UpdateProcessStates.UPDATESTATUSINSTALLING
		if !result.IsValid() {
			return nil
		}
		return &result
	case 9:
		result := UpdateProcessStates.UPDATESTATUSINSTALLCOMPLETE
		if !result.IsValid() {
			return nil
		}
		return &result
	case 10:
		result := UpdateProcessStates.UPDATESTATUSERROR
		if !result.IsValid() {
			return nil
		}
		return &result
...
type UpdateProcessStateMatcher interface {
	UpdateStatusIdle()
	UpdateStatusChecking()
	UpdateStatusNoUpgrade()
	UpdateStatusUpgradeAvailable()
	UpdateStatusDownloading()
	UpdateStatusDownloadComplete()
	UpdateStatusExtracting()
	UpdateStatusExtractComplete()
	UpdateStatusInstalling()
	UpdateStatusInstallComplete()
	UpdateStatusError()
}

// MatchUpdateProcessState dispatches to the matcher method for the given enum value.
// The UpdateProcessStateMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchUpdateProcessState(en UpdateProcessState, matcher UpdateProcessStateMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil UpdateProcessStateMatcher")
	}
	switch en {
	case UpdateProcessStates.UPDATESTATUSIDLE:
		matcher.UpdateStatusIdle()
	case UpdateProcessStates.UPDATESTATUSCHECKING:
		matcher.UpdateStatusChecking()
	case UpdateProcessStates.UPDATESTATUSNOUPGRADE:
		matcher.UpdateStatusNoUpgrade()
	case UpdateProcessStates.UPDATESTATUSUPGRADEAVAILABLE:
		matcher.UpdateStatusUpgradeAvailable()
	case UpdateProcessStates.UPDATESTATUSDOWNLOADING:
		matcher.UpdateStatusDownloading()
	case UpdateProcessStates.UPDATESTATUSDOWNLOADCOMPLETE:
		matcher.UpdateStatusDownloadComplete()
	case UpdateProcessStates.UPDATESTATUSEXTRACTING:
		matcher.UpdateStatusExtracting()
	case UpdateProcessStates.UPDATESTATUSEXTRACTCOMPLETE:
		matcher.UpdateStatusExtractComplete()
	case UpdateProcessStates.UPDATESTATUSINSTALLING:
		matcher.UpdateStatusInstalling()
	case UpdateProcessStates.UPDATESTATUSINSTALLCOMPLETE:
		matcher.UpdateStatusInstallComplete()
	case UpdateProcessStates.UPDATESTATUSERROR:
		matcher.UpdateStatusError()
	default:
		return fmt.Errorf("unhandled UpdateProcessState: %v", en)
	}
	return nil
}

// MustMatchUpdateProcessState dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchUpdateProcessState(en UpdateProcessState, matcher UpdateProcessStateMatcher) {
	if err := MatchUpdateProcessState(en, matcher); err != nil {
		panic(err)
	}
}
...
func (u *UpdateProcessState) UnmarshalJSON(by []byte) error {
	var value string
	...
	newu, err := ParseUpdateProcessState(by)
	if err != nil {
		return err
	}
	*u = newu
...
func (u *UpdateProcessState) UnmarshalText(by []byte) error {
	newu, err := ParseUpdateProcessState(by)
	if err != nil {
		return err
	}
...
// It returns an error if the byte slice does not contain a valid enum value.
func (u *UpdateProcessState) UnmarshalBinary(by []byte) error {
	newu, err := ParseUpdateProcessState(by)
	if err != nil {
		return err
	}
...
func (u *UpdateProcessState) UnmarshalYAML(by []byte) error {
	newu, err := ParseUpdateProcessState(by)
	if err != nil {
		return err
	}
...
// updateprocessstateNames is a constant string containing the canonical names for all enum values.
