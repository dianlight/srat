...
// It is private; use the exported SmartAttributeCode values and methods instead.
...
// Otherwise, it returns nil
func numberToSmartAttributeCode[T interface {
	int | int8 | int16 | int32 | int64 | uint | uint8 | uint16 | uint32 | uint64 | uintptr | float32 | float64
}](num T) *SmartAttributeCode {
	f := float64(num)
	if math.Floor(f) != f {
		return nil
	}
	switch f {
	case 0:
		result := SmartAttributeCodes.SMARTATTRIBUTEUNDEFINED
		if !result.IsValid() {
			return nil
		}
		return &result
	case 1:
		result := SmartAttributeCodes.SMARTATTRTEMPERATURECELSIUS
		if !result.IsValid() {
			return nil
		}
		return &result
	case 2:
		result := SmartAttributeCodes.SMARTATTRPOWERONHOURS
		if !result.IsValid() {
			return nil
		}
		return &result
	case 3:
		result := SmartAttributeCodes.SMARTATTRPOWERCYCLECOUNT
		if !result.IsValid() {
			return nil
		}
		return &result
...
type SmartAttributeCodeMatcher interface {
	SmartAttributeUndefined()
	SmartAttrTemperatureCelsius()
	SmartAttrPowerOnHours()
	SmartAttrPowerCycleCount()
}

// MatchSmartAttributeCode dispatches to the matcher method for the given enum value.
// The SmartAttributeCodeMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchSmartAttributeCode(en SmartAttributeCode, matcher SmartAttributeCodeMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil SmartAttributeCodeMatcher")
	}
	switch en {
	case SmartAttributeCodes.SMARTATTRIBUTEUNDEFINED:
		matcher.SmartAttributeUndefined()
	case SmartAttributeCodes.SMARTATTRTEMPERATURECELSIUS:
		matcher.SmartAttrTemperatureCelsius()
	case SmartAttributeCodes.SMARTATTRPOWERONHOURS:
		matcher.SmartAttrPowerOnHours()
	case SmartAttributeCodes.SMARTATTRPOWERCYCLECOUNT:
		matcher.SmartAttrPowerCycleCount()
	default:
		return fmt.Errorf("unhandled SmartAttributeCode: %v", en)
	}
	return nil
}

// MustMatchSmartAttributeCode dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchSmartAttributeCode(en SmartAttributeCode, matcher SmartAttributeCodeMatcher) {
	if err := MatchSmartAttributeCode(en, matcher); err != nil {
		panic(err)
	}
}
...
// smartattributecodeNames is a constant string containing the canonical names for all enum values.
