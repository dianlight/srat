...
func TestClientOptions(t *testing.T) {
	...
	client, err := NewClient("http://mount.local", WithHTTPClient(mock), WithRequestEditorFn(func(ctx context.Context, req *http.Request) error {
		editorCalled = true
		req.Header.Set("X-Mount", "1")
		return nil
	}))
	...
	assert.Equal(t, "1", mock.lastReq.Header.Get("X-Mount"))
	assert.True(t, editorCalled)

	additionalCalled := false
	req, err := NewGetMountsRequest(client.Server)
	require.NoError(t, err)
	...
	overrideClient, err := NewClient("http://mount.local", WithBaseURL("http://override.local"))
	require.NoError(t, err)
