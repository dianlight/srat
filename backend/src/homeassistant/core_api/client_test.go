...
func TestClientOptions(t *testing.T) {
	...
	client, err := NewClient("http://api.local", WithHTTPClient(mock), WithRequestEditorFn(func(ctx context.Context, req *http.Request) error {
		editorCalled = true
		req.Header.Set("X-Api", "1")
		return nil
	}))
	...
	assert.Equal(t, "1", mock.lastReq.Header.Get("X-Api"))
	assert.True(t, editorCalled)

	additionalCalled := false
	req, err := NewGetApiRequest(client.Server)
	require.NoError(t, err)
	...
	overrideClient, err := NewClient("http://api.local", WithBaseURL("http://override.local"))
	require.NoError(t, err)
