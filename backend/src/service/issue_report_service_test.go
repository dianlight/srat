...
func (suite *IssueReportServiceSuite) TestGenerateIssueReport_SambaProblem_WithSensitiveDiagnosticsSmall() {
	...
	suite.Contains(params.Get("srat_config"), "password=Secret123")
	suite.Contains(params.Get("srat_config"), "token=tok_123")

	suite.Contains(params.Get("addon_config"), "user@example.com")
	suite.Contains(params.Get("addon_config"), "\"password\":\"Secret123\"")
	suite.Contains(params.Get("addon_config"), "\"token\":\"tok_123\"")

	suite.Contains(params.Get("logs"), "password=Secret123")
	suite.Contains(params.Get("logs"), "email=user@example.com")
	suite.Contains(params.Get("logs"), "token=tok_123")

	suite.Contains(params.Get("console"), "password=Secret123")
	suite.Contains(params.Get("console"), "email=user@example.com")
	suite.Contains(params.Get("console"), "token=tok_123")
...
func (suite *IssueReportServiceSuite) TestGenerateIssueReport_SambaProblem_WithSensitiveDiagnosticsLarge() {
	...
	suite.Contains(gistRequestBody, "password=Secret123")
	suite.Contains(gistRequestBody, "email=user@example.com")
	suite.Contains(gistRequestBody, "token=tok_123")
