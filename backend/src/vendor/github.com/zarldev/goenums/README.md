...
- [Features Expanded](#features-expanded)
  ...
  - [Compile-time Exhaustiveness](#compile-time-exhaustiveness)
...
### Selecting enum accessor style

By default, goenums preserves the historical upper-case container accessor style:

```go
Statuses.PENDING_APPROVAL
```

New projects that prefer idiomatic exported Go selectors can opt in to Go-style accessors:
...
Changing accessor style requires updating Go call sites, but it does not change enum aliases, parsing, serialized values, database values, or generated filenames.
...
For true compile-time exhaustiveness checking, goenums generates a `Matcher` interface and `Match`/`MustMatch` functions. The interface has one method per valid enum value, so any type that implements it is guaranteed to handle every case:
...
// Compile-time check: planetHandler must satisfy PlanetMatcher.
// If a new planet is added, this line fails to compile until the
// corresponding method is added to planetHandler.
var _ solarsystem.PlanetMatcher = planetHandler{}

func handlePlanet(p solarsystem.Planet) error {
    // Use Match for runtime dispatch with error handling.
    if err := solarsystem.MatchPlanet(p, planetHandler{}); err != nil {
        // matcher was nil or the enum value was not handled
        return err
    }

    // Use MustMatch when you want a panic instead of an error.
    solarsystem.MustMatchPlanet(p, planetHandler{})
    return nil
}
```

The `Match` function returns an error if the matcher is `nil` or the enum value is not one of the valid generated cases. The `MustMatch` function panics instead. Both dispatch to the appropriate method on the matcher based on the enum value.

This is especially useful in tests to ensure all enum values are covered, and in production code where missing a case could cause subtle bugs. The compile-time check (`var _ XMatcher = myMatcher{}`) guarantees that the matcher type fully implements the generated interface before the code even compiles.
...
```go
// DO NOT EDIT.
// code generated by goenums v0.7.0
